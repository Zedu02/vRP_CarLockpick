...
AddEventHandler('hjalte:open', function()
    ...
    TriggerEvent("pNotify:SendNotification",{text = "Nici un vehicul in preajma!",type = "warning",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer=true})
	end
end)
...
Citizen.CreateThread(function()
	while true do
      ...
      DrawText3Ds(coords.x, coords.y, coords.z-0.65, "[~p~Q~s~] - Foloseste Lockpick~n~[~p~H~s~] - Fixeaza-l in aceasta pozitie.~n~[~p~X~s~] - Anuleaza")
	...
	while true do
		...
		if open then
			...
			elseif pin1 and pin2 and pin3 and pin4 then
				...
				DisableControlAction(0, 73, false)
        TriggerEvent("pNotify:SendNotification",{text = "Ai deschis masina",type = "info",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer=true})
